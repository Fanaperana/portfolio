<script lang="ts">
  ...
  function setTheme(name: string) {
    ...
    const themes: Record<
      string,
      { accent: string; soft: string; link: string }
    > = {
  ...
  async function submit() {
    ...
    if (!cmd) {
      ...
      push({
        kind: "text",
        value: `${name}: command not found — type \`help\``,
        tone: "error",
      });
    } else {
      try {
        ...
        push({
          kind: "text",
          value: `${cmd.name}: ${(e as Error).message}`,
          tone: "error",
        });
      }
    }
  ...
  onMount(() => {
    ...
    // Focus without scrolling so the page does not jump to the terminal on load.
  ...
  <div class="flex flex-wrap items-end justify-between gap-4">
    <div>
      ...
      <p class="mt-2 max-w-2xl text-muted-foreground">
        A real parser in Rust, compiled to WASM, evaluates your pipelines
        in-browser. Type <code
          class="rounded bg-card px-1.5 py-0.5 font-mono text-xs text-primary"
          >help</code
        >
        to see commands. Press
        <kbd
          class="rounded border border-border bg-card px-1.5 py-0.5 font-mono text-xs"
          >?</kbd
        >
        anywhere to focus.
      </p>
      ...
      >
        {#each lines as line, i (i)}
          {#if line.prompt !== undefined}
            <div
              class="flex gap-2 break-words whitespace-pre-wrap text-foreground"
            >
              <span class="flex-none text-primary">
                {PROFILE.login.toLowerCase()}@portfolio<span
          ...
          {:else if line.out}
            ...
            {:else if o.kind === "link"}
              <a
                ...
                href={o.href}
                target="_blank"
                rel="noreferrer">{o.label} ↗</a
              >
            {:else if o.kind === "projects"}
              <div class="my-1 grid gap-1 pl-1">
                {#each o.value as p (p.slug)}
                  <div class="flex items-start gap-3">
                    <span class="w-28 flex-none truncate text-primary"
                      >{p.slug}</span
                    >
                    <span
                      class="w-24 flex-none truncate text-muted-foreground/70"
                      >{p.language}</span
                    >
                    <span
                      class="w-10 flex-none text-right text-muted-foreground/70"
                      >★ {p.stars}</span
                    >
