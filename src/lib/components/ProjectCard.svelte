<script lang="ts">
  ...
  const gradient = $derived(
    gradients[project.accent ?? "teal"] ?? gradients.teal
  );
  ...
  >
    ...
    >
      {#if thumbSrc}
        ...
        />
        <div
          class="absolute inset-0 bg-gradient-to-t from-card/70 via-card/10 to-transparent"
        ></div>
      {:else}
        <div
        ...
        <div class="relative flex h-full w-full items-center justify-center">
          <span
            class="font-mono text-7xl text-foreground opacity-70 drop-shadow-lg"
          >
            {project.glyph ?? "▢"}
          </span>
        </div>
      ...
      {#if project.highlight}
        <Badge
          class="absolute top-2 right-2 bg-primary font-mono text-[10px] text-primary-foreground"
        >
          featured
        </Badge>
      {:else}
    ...
    <div class="flex flex-1 flex-col gap-3 p-4">
      <div class="flex items-start justify-between gap-2">
        <h3
          class="font-mono text-base font-semibold text-foreground group-hover:text-primary"
        >
          {project.title}
        </h3>
      ...
      <div class="flex flex-wrap gap-2">
        ...
        {#if project.homepage}
          ...
          >
            ...
            <span
              >visit {new URL(project.homepage).host.replace(
                /^www\./,
                ""
              )}</span
            >
